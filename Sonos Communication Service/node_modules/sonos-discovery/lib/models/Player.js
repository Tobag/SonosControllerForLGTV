...
var streamer = require('../helpers/streamer');
var TYPE = soap.TYPE;
...
var EMPTY_STATE = require('../types/empty-state');
var PLAY_MODE = require('../types/play-mode');
var REPEAT_MODE = require('../types/repeat-mode');

function reversePlayMode() {
  var lookup = {};
  for (var key in PLAY_MODE) {
...
var PLAY_MODE_LOOKUP = Object.freeze(reversePlayMode());

function getPlayMode(state) {
  var key = state.shuffle << 1 | (state.repeat === REPEAT_MODE.ONE ? 4 : state.repeat === REPEAT_MODE.ALL ? 1 : 0);
...
function parseTrackMetadata(metadata, nextTrack) {
  return new Promise(function (resolve, reject) {
    var track = nextTrack ? clone(EMPTY_STATE.nextTrack) : clone(EMPTY_STATE.currentTrack);
    if (!metadata) resolve(track);

    var sax = flow(streamer(metadata.val));

    sax.on('tag:item', function (item) {
...
function getState(playerInternal, coordinatorInternal) {
  var diff = 0;
  if (coordinatorInternal.playbackState === 'PLAYING') diff = Date.now() - coordinatorInternal.stateTime;
...
function Player(data, listener, system) {
  ...
  Object.defineProperty(_this, 'state', {
    get: function get() {
      return getState(state, _this.coordinator._state);
    }
  });
  ...
  var uri = url.parse(data.location);
  _this.baseUrl = uri.protocol + '//' + uri.host;

  var subscribeEndpoints = ['/MediaRenderer/AVTransport/Event', '/MediaRenderer/RenderingControl/Event', '/MediaRenderer/GroupRenderingControl/Event', '/MediaServer/ContentDirectory/Event'];

  var subscriptions = subscribeEndpoints.map(function (path) {
  ...
  function getPositionInfo() {
    return soap.invoke(_this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.GetPositionInfo).then(soap.parse).then(function (response) {
  ...
  function notificationHandler(uuid, data) {
    ...
    if (data.transportstate) {
      ...
      var currentPlayMode = PLAY_MODE[data.currentplaymode.val];
      ...
      parseTrackMetadata(data.currenttrackmetadata).then(function (track) {
        ...
        if (data.avtransporturimetadata && data.avtransporturimetadata.val) {
          return parseAVTransportMetadata(data.avtransporturimetadata).then(function (radioInfo) {
        ...
        return musicServices.tryGetHighResArt(state.currentTrack.uri).then(function (highResAlbumArtUrl) {
        ...
        return parseTrackMetadata(data['r:nexttrackmetadata'], true);
      }).then(function (track) {
        state.nextTrack = track;
        if (track.uri) {
          return musicServices.tryGetHighResArt(state.nextTrack.uri).then(function (highResAlbumArtUri) {
        ...
        if (!_this.avTransportUri.startsWith('x-rincon:') && _this.state.playbackState !== 'TRANSITIONING') {
          // Only fetch position info if coordinator
          return getPositionInfo().then(function () {
    ...
    if (data.mute) {
      var master = data.mute.find(function (x) {
        return x.channel === 'Master';
      });
      var previousMute = state.mute;
    ...
    if (data.volume) {
      var _master = data.volume.find(function (x) {
        return x.channel === 'Master';
      });
      var previousVolume = state.volume;
      state.volume = parseInt(_master.val);
  ...
  function groupMuteHandler(uuid, mute) {
    ...
    var previousMute = _this.groupState.mute;
...
Player.prototype.play = function play() {
  ...
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Play);
};

Player.prototype.pause = function pause() {
  logger.debug('invoking pause');
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Pause);
};

Player.prototype.stop = function stop() {
  logger.debug('invoking stop');
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Stop);
};

Player.prototype.nextTrack = function nextTrack() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Next);
};

Player.prototype.previousTrack = function previousTrack() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Previous);
};

Player.prototype.mute = function mute() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/RenderingControl/Control', TYPE.Mute, { mute: 1 });
};

Player.prototype.unMute = function unMute() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/RenderingControl/Control', TYPE.Mute, { mute: 0 });
};

Player.prototype.unMuteGroup = function unMuteGroup() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/GroupRenderingControl/Control', TYPE.GroupMute, { mute: 0 });
};

Player.prototype.muteGroup = function muteGroup() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/GroupRenderingControl/Control', TYPE.GroupMute, { mute: 1 });
};
...
Player.prototype.setVolume = function setVolume(level) {
  ...
  return soap.invoke(this.baseUrl + '/MediaRenderer/RenderingControl/Control', TYPE.Volume, { volume: level });
};

Player.prototype.timeSeek = function timeSeek(seconds) {
  var formattedTime = formatTime(seconds);
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Seek, { unit: 'REL_TIME', value: formattedTime });
};

Player.prototype.trackSeek = function trackSeek(trackNo) {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.Seek, { unit: 'TRACK_NR', value: trackNo });
};

Player.prototype.clearQueue = function clearQueue() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.RemoveAllTracksFromQueue);
};

Player.prototype.removeTrackFromQueue = function removeTrackFromQueue(index) {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.RemoveTrackFromQueue, { track: index || 0 });
};

Player.prototype.removeTrackRangeFromQueue = function removeTrackRangeFromQueue(startIndex, numberOfTracks) {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.RemoveTrackRangeFromQueue, {
...
Player.prototype.reorderTracksInQueue = function reorderTracksInQueue(startIndex, numberOfTracks, insertBefore) {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.ReorderTracksInQueue, {
    startIndex: parseInt(startIndex, 10) || 0,
    numberOfTracks: parseInt(numberOfTracks, 10) || 0,
...
Player.prototype.saveQueue = function saveQueue(title) {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.SaveQueue, {
    title: title
  });
};
...
Player.prototype.addURIToQueue = function addURIToQueue(uri, metadata, enqueueAsNext, desiredFirstTrackNumberEnqueued) {
  ...
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.AddURIToQueue, {
    uri: uri,
    metadata: metadata,
...
Player.prototype.setPlayMode = function setPlayMode(newPlayMode) {
  ...
  if (newPlayMode.repeat !== undefined || newPlayMode.shuffle !== undefined) {
    var desiredPlayMode = Object.assign({}, this.state.playMode, newPlayMode);
    var playMode = getPlayMode(desiredPlayMode);

    logger.debug('repeat: ' + newPlayMode.repeat + ', shuffle: ' + newPlayMode.shuffle);
    logger.debug('calculated playmode to ' + playMode);
    promise = promise.then(function () {
      return soap.invoke(_this2.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.SetPlayMode, { playMode: playMode }).catch(function (err) {
  ...
  if (newPlayMode.crossfade !== undefined) {
    promise = promise.then(function () {
      return soap.invoke(_this2.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.SetCrossfadeMode, { crossfadeMode: newPlayMode.crossfade ? 1 : 0 });
    });
  }
...
Player.prototype.sleep = function sleep(seconds) {
  ...
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.ConfigureSleepTimer, { time: formattedTime });
};
...
Player.prototype.setAVTransport = function setAVTransport(uri, metadata) {
  ...
  var entityEncodedMetadata = xmlEntities.encode(metadata);
  var entityEncodedUri = xmlEntities.encode(uri);
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.SetAVTransportURI, { uri: entityEncodedUri, metadata: entityEncodedMetadata });
};

Player.prototype.becomeCoordinatorOfStandaloneGroup = function becomeCoordinatorOfStandaloneGroup() {
  return soap.invoke(this.baseUrl + '/MediaRenderer/AVTransport/Control', TYPE.BecomeCoordinatorOfStandaloneGroup);
};

Player.prototype.refreshShareIndex = function refreshShareIndex() {
  return soap.invoke(this.baseUrl + '/MediaServer/ContentDirectory/Control', TYPE.RefreshShareIndex);
};

Player.prototype.browse = function browse(objectId, startIndex, limit) {
  startIndex = startIndex === undefined ? 0 : startIndex;
  limit = limit === undefined ? 0 : limit;
  return soap.invoke(this.baseUrl + '/MediaServer/ContentDirectory/Control', TYPE.Browse, { objectId: objectId, startIndex: startIndex, limit: limit }).then(soap.parse).then(function (res) {
    return new Promise(function (resolve, reject) {
      var returnResult = {
      ...
      var stream = streamer(res.result);
      ...
      sax.on('tag:item', function (item) {
        returnResult.items.push({
          ...
          albumArtUri: item['upnp:albumarturi'] instanceof Array ? item['upnp:albumarturi'][0] : item['upnp:albumarturi'],
          metadata: item['r:resmd']
        });
      });

      sax.on('tag:container', function (item) {
...
Player.prototype.browseAll = function browseAll(objectId) {
  ...
  var getChunk = function getChunk(chunk) {
    ...
    return _this3.browse(objectId, chunk.startIndex + chunk.numberReturned, 0).then(getChunk);
  };
...
requireDir(path.join(__dirname, '../prototypes/Player'), function (proto) {
