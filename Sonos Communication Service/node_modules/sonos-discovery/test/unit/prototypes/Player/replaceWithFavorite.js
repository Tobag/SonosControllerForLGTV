...
describe('Player.replaceWithFavorite', function () {
  var replaceWithFavorite = require('../../../../lib/prototypes/Player/replaceWithFavorite.js');

  describe('When replacing with streaming favorite', function () {
    var player = void 0;
    var system = void 0;
    ...
    it('Has fetched favorites', function () {
      expect(system.getFavorites).calledOnce;
    });

    it('Removes all tracks before adding them', function () {
      expect(player.clearQueue).calledOnce;
    });

    it('Adds URI to queue with metadata', function () {
      expect(player.addURIToQueue).calledOnce;
      expect(player.addURIToQueue.firstCall.args).eql([favorites[0].uri, favorites[0].metadata]);
    });

    it('Sets the avtransport to current queue', function () {
      expect(player.setAVTransport).calledOnce;
      expect(player.setAVTransport.firstCall.args[0]).equal('x-rincon-queue:' + player.uuid + '#0');
    });
  });

  describe('When replacing with radio favorite', function () {
    var player = void 0;
    var system = void 0;
    var favorites = void 0;
    ...
    it('Do not remove tracks', function () {
      expect(player.clearQueue).not.called;
    });

    it('Do not try to add uri to queue', function () {
      expect(player.addURIToQueue).not.called;
    });

    it('Sets the avtransport to uri directly', function () {
      expect(player.setAVTransport).calledOnce;
  ...
  describe('When favorite is not found', function () {
    var player = void 0;
    var system = void 0;
    var favorites = void 0;
    var success = void 0;
    ...
    before(function () {
      ...
      return replaceWithFavorite.call(player, 'some favorite').then(success).catch(fail);
    });

    it('Promise should not resolve', function () {
      expect(success).not.called;
    });

    it('Promise should reject', function () {
