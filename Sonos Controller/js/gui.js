...
var GUI = {
	masterVolume: new VolumeSlider(document.getElementById('master-volume'), function (volume) {
			EventActions.sendToSonosService('group-volume', {uuid: Sonos.currentState.selectedZone, volume: volume});
		}, function (obj) {
	...
	progress: new ProgressBar(document.getElementById('position-bar'), function (position) {
		...
		EventActions.sendToSonosService('track-seek', {uuid: player.uuid, elapsed: desiredElapsed});
	})
};