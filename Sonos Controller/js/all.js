"use strict";
var EventActions = {};
EventActions.topologyChanged = function (shouldRenderVolumes) {
...
EventActions.volumeChanged = function (data) {
  Sonos.players[data.uuid].state.volume = data.newVolume;
  GUI.playerVolumes[data.uuid].setVolume(data.newVolume);

};

EventActions.groupMuteChanged = function (data) {
  updateControllerState();
};

EventActions.muteChanged = function (data) {
...
EventActions.favoritesChanged = function (data) {
  renderFavorites(data);
};

EventActions.queueChanged = function (data) {
  if (data.uuid != Sonos.currentState.selectedZone) return;
  renderQueue(data.queue);
};

EventActions.searchResultReceived = function (data) {
  renderSearchResult(data);
};

EventActions.sendToSonosService = function(message, payload){
  sonosService.sendMessage(message, payload);
};
...
var zoneManagement = function () {
  ...
  function handleDrop(e) {
    if (e.target.parentNode == this) {
      // detach
      EventActions.sendToSonosService('group-management', { player: dragItem.dataset.id, group: null });
      return;
    }
    ...
    EventActions.sendToSonosService('group-management', { player: dragItem.dataset.id, group: zone.id });
...
function renderVolumes() {
  ...
  playerNodes.forEach(function (playerPair) {
    ...
    GUI.playerVolumes[uuid] = new VolumeSlider(node, function (vol) {
      EventActions.sendToSonosService('volume', { uuid: uuid, volume: vol });
    });
